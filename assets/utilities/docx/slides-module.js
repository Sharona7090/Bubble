...
var SlidesRepo = __webpack_require__(/*! ./slidesrepo.js */ "./es6/slidesrepo.js");

var Docxtemplater = __webpack_require__(/*! docxtemplater */ "./node_modules/docxtemplater/js/docxtemplater.js");

var moduleName = "pro-xml-templating/slides-module-repeat";
var slideNumRegex = /ppt\/slides\/slide([0-9]+).xml/;

var createScope = __webpack_require__(/*! docxtemplater/js/scope-manager.js */ "./node_modules/docxtemplater/js/scope-manager.js");

var _require2 = __webpack_require__(/*! ./utils.js */ "./es6/utils.js"),
    getPathToSlide = _require2.getPathToSlide;
...
var _require3 = __webpack_require__(/*! ./utils.js */ "./es6/utils.js"),
    typeAS = _require3.typeAS;

var SlidesModule = /*#__PURE__*/function () {
  function SlidesModule() {
    _classCallCheck(this, SlidesModule);

    this.requiredAPIVersion = "3.26.0";
    this.slidesRepo = new SlidesRepo();
    this.customMap = [];
    this.name = "SlidesModule";
    this.supportedFileTypes = ["pptx"];
    this.prefix = ":";

    if (this.baseTags != null) {
      delete this.baseTags;
    }
  }

  _createClass(SlidesModule, [{
    key: "set",
    value: function set(_ref) {
      var data = _ref.data,
          zip = _ref.zip,
          xmlDocuments = _ref.xmlDocuments,
      ...
      if (zip) {
        this.slidesRepo.zip = zip;

        if (this.docxtemplater.slidesRepo) {
          this.slidesRepo = this.docxtemplater.slidesRepo;
        }

        this.docxtemplater.slidesRepo = this.slidesRepo;

        if (this.docxtemplater.slidesCustomMap) {
          this.customMap = this.docxtemplater.slidesCustomMap;
        }
      }

      if (xmlDocuments) {
        this.slidesRepo.xmlDocuments = xmlDocuments;
      }

      if (inspect && inspect.filePath) {
        this.customMap[getSlideIndex(inspect.filePath)] = null;
      }
    }
  }, {
    key: "parse",
    value: function parse(placeHolderContent, _ref2) {
      ...
      if (match(prefix, placeHolderContent)) {
        var _module = moduleName;
        return {
          type: type,
          value: getValue(prefix, placeHolderContent),
          module: _module
        };
      }
    ...
    value: function postparse(parsed, options) {
      ...
      var errors = [];
      var slideIndex = getSlideIndex(options.filePath);
      var tags = [];
      var postparsed = parsed.filter(function (p) {
        if (p.module !== moduleName) {
          return true;
        }

        if (options.contentType === typeAS) {
          tags.push(p);
          _this.customMap[slideIndex] = p;
        }
      ...
      if (tags.length > 1) {
        var err = new Docxtemplater.Errors.XTTemplateError("Multiple slide tags on one single slide for slide ".concat(slideIndex + 1));
        err.properties = {
          explanation: "The slide ".concat(slideIndex + 1, " has multiple {:tags}, which is not allowed on a single slide"),
    ...
    value: function optionsTransformer(options, docxtemplater) {
      ...
      docxtemplater.zip.file(/ppt\/notesSlides\/notesSlide\d+\.xml/).map(function (file) {
        docxtemplater.targets.push(file.name);
      });
      var fileNames = docxtemplater.zip.file(/\[Content_Types\].xml/).concat(docxtemplater.zip.file(/ppt\/_rels\/presentation.xml.rels/)).concat(docxtemplater.zip.file(/ppt\/presentation.xml/)).concat(docxtemplater.zip.file(/ppt\/slides\/_rels\/.*\.xml\.rels/)).concat(docxtemplater.zip.file(/ppt\/slideMasters\/_rels\/.*\.xml\.rels/)).map(function (file) {
        return file.name;
      });
      this.docxtemplater = docxtemplater;
      options.xmlFileNames = options.xmlFileNames.concat(fileNames);
      return options;
    }
  }, {
    key: "getSlides",
    value: function getSlides(baseData) {
      var _this2 = this;
      ...
      return this.customMap.reduce(function (slides, part, slideNum) {
        if (_this2.docxtemplater.slidesGetter) {
          var newSlides = _this2.docxtemplater.slidesGetter(slides, part, slideNum, baseData);
        ...
        var sm = createScope({
          tags: baseData,
          parser: _this2.docxtemplater.parser
        });
        var value = sm.getValue(part.value, {
          part: part
        });
        ...
        if (isArray(value)) {
          if (typeof part.chunkBy === "number") {
            chunk(value, part.chunkBy).forEach(function (chunk) {
              slides.push(merge({}, baseData, {
                $chunk: chunk,
                $slide: slideNum + 1,
                $part: part
    ...
    key: "getRenderedMap",
    value: function getRenderedMap(map) {
      var _this3 = this;
      ...
      map = Object.keys(map).reduce(function (newMap, key) {
        if (key.indexOf("ppt/slides/") !== -1 || key.indexOf("ppt/notesSlides/") !== -1) {
          return newMap;
        }
      ...
      var baseData = omit(clone(this.data), ["slides"]);

      if (!this.slidesRepo.existingSlides) {
        this.slidesRepo.loadSlides();
        this.slidesRepo.removeAllSlides();
      }

      var slides = this.getSlides(baseData);
      var slideCount = slides.length;
      this.map = slides.reduce(function (map, data, slideIndex) {
        data.$index = slideIndex + 1;
        data.$count = slideCount;

        var newMap = _this3.slidesRepo.addSlide(data.$index, data.$slide, data);

        var from = getPathToSlide(data.$slide);
        var to = getPathToSlide(slideIndex + 1);

        if (data.$part) {
          _this3.docxtemplater.scopeManagers[to] = createScope({
            tags: baseData,
            parser: _this3.docxtemplater.parser,
            to: to,
            resolveOffset: 1
          }).createSubScopeManager(data, data.$part.value, 0, data.$part, 1);
        } else {
...
var _require2 = __webpack_require__(/*! docxtemplater/js/doc-utils.js */ "./node_modules/docxtemplater/js/doc-utils.js"),
    str2xml = _require2.str2xml,
...
function getByAttribute(doc, name, value) {
  var relationships = doc.getElementsByTagName("Relationship");
  return Array.prototype.find.call(relationships, function (rel) {
    return rel.getAttribute(name) === value;
  });
}

var _require3 = __webpack_require__(/*! ./utils.js */ "./es6/utils.js"),
    getPathToSlide = _require3.getPathToSlide,
    getPathToRelSlide = _require3.getPathToRelSlide,
    getPathToNote = _require3.getPathToNote,
    getPathToNoteRel = _require3.getPathToNoteRel,
    getTargetSlide = _require3.getTargetSlide,
    typeNS = _require3.typeNS,
    typeSl = _require3.typeSl,
    typeAR = _require3.typeAR,
    typeANS = _require3.typeANS,
    typeAS = _require3.typeAS;

var SlidesRepository = /*#__PURE__*/function () {
  function SlidesRepository() {
    _classCallCheck(this, SlidesRepository);
  ...
  _createClass(SlidesRepository, [{
    key: "removeElement",
    value: function removeElement(baseElements, tagName, attribute, value) {
      var nodes = Array.prototype.slice.call(baseElements.getElementsByTagName(tagName));
      return find(nodes, function (node) {
        if (node.getAttribute == null) {
          return false;
        }

        var partName = node.getAttribute(attribute);

        if (partName === value || partName.indexOf(value) !== -1) {
          baseElements.removeChild(node);
          return true;
        }
      });
    }
    ...
    value: function getNextRid() {
      this.currentRid++;
      var stringifiedRid = "rId" + this.currentRid.toString();

      if (this.ridArray.indexOf(stringifiedRid) !== -1) {
        return this.getNextRid();
      }

      this.ridArray.push(stringifiedRid);
      return stringifiedRid;
    }
  }, {
    key: "fileExists",
    ...
    key: "deleteAndSaveFile",
    value: function deleteAndSaveFile(filename) {
      var content = this.zip.file(filename).asText();
      this.zip.remove(filename);
      delete this.xmlDocuments[filename];
      return content;
    }
  }, {
    key: "removeOverride",
    value: function removeOverride(Target) {
      delete this.overrides[Target];
      return this.removeElement(this.types, "Override", "PartName", Target);
    }
  }, {
    key: "removeTarget",
    value: function removeTarget(Target) {
      return this.removeElement(this.relationships, "Relationship", "Target", Target);
    }
  }, {
    key: "removeSlide",
    value: function removeSlide(slideNum) {
      this.zip.remove(getPathToSlide(slideNum));
      var relSlidePath = getPathToRelSlide(slideNum);
      var relContent = this.deleteAndSaveFile(relSlidePath);
      var relXml = str2xml(relContent);
      var noteRel = this.removeElement(relXml.getElementsByTagName("Relationships")[0], "Relationship", "Type", typeNS);

      if (noteRel) {
        var noteTarget = noteRel.getAttribute("Target");
        var notesNum = noteTarget.replace(/^\.\.\/notesSlides\/notesSlide([0-9]+)\.xml$/, "$1");
        var notesPath = "ppt/notesSlides/notesSlide".concat(notesNum, ".xml");
        var notesRelsPath = "ppt/notesSlides/_rels/notesSlide".concat(notesNum, ".xml.rels");
        this.removeOverride("/".concat(notesPath));
        this.noteContent[slideNum] = this.deleteAndSaveFile(notesPath);
        this.notePath[slideNum] = notesPath;
        this.noteRelContent[slideNum] = this.deleteAndSaveFile(notesRelsPath);
      }

      this.relContent[slideNum] = xml2str(relXml);
      var slidePath = getPathToSlide(slideNum);
      this.removeOverride("/".concat(relSlidePath));
      this.removeOverride("/".concat(slidePath));
      var el = this.removeTarget(getTargetSlide(slideNum));
      var rId = el.getAttribute("Id");
      this.removeElement(this.slideIdList, "p:sldId", "r:id", rId);
    }
  }, {
    key: "addOverride",
    value: function addOverride(data) {
      if (this.overrides[data.PartName]) {
        return;
      }

      this.overrides[data.PartName] = data.ContentType;
      var newTag = this.contentTypes.createElement("Override");
      setAttributes(newTag, data);
      this.types.appendChild(newTag);
    }
  }, {
    key: "addSlide",
    value: function addSlide(slideNum, fromSlideNum, data) {
      var map = {};
      var noteContent = this.noteContent[fromSlideNum];
      var notesPath = getPathToNote(slideNum);

      if (noteContent) {
        map[notesPath] = {
          from: this.notePath[fromSlideNum],
          data: data
        };
      }

      if (this.addedSlides[slideNum]) {
        return map;
      }

      this.addedSlides[slideNum] = true;

      if (fromSlideNum == null) {
        throw new Error("Adding slide from null slide ".concat(fromSlideNum));
      }

      var relContent = this.relContent[fromSlideNum];

      if (!relContent) {
        var err = new Docxtemplater.Errors.RenderingError("The slide ".concat(fromSlideNum, " does not exist"));
        err.properties = {
          explanation: "The slide ".concat(fromSlideNum, " does not exist in your template, please check your slides data"),
          id: "slide_does_not_exist",
          slide: fromSlideNum
        };
        throw err;
      }

      var relationsSlides = str2xml(relContent);
      this.addOverride({
        PartName: "/".concat(getPathToRelSlide(slideNum)),
        ContentType: typeAR
      });

      if (noteContent) {
        this.addOverride({
          PartName: "/".concat(notesPath),
          ContentType: typeANS
        });
        var notesRelsPath = getPathToNoteRel(slideNum);
        var noteRels = str2xml(this.noteRelContent[fromSlideNum]);
        var rel = getByAttribute(noteRels, "Type", typeSl);

        if (rel) {
          setAttributes(rel, {
            Target: "../slides/slide".concat(slideNum, ".xml")
          });
        }

        this.xmlDocuments[notesRelsPath] = noteRels;
        var noteRel = relationsSlides.createElement("Relationship");
        relationsSlides.getElementsByTagName("Relationships")[0].appendChild(noteRel);
        setAttributes(noteRel, {
          Id: "".concat(this.getNextRid()),
          Type: typeNS,
          Target: "../notesSlides/notesSlide".concat(slideNum, ".xml")
        });
      }

      this.addOverride({
        PartName: "/".concat(getPathToSlide(slideNum)),
        ContentType: typeAS
      });
      var newTag = this.presentationRels.createElement("Relationship");
      var curRid = "".concat(this.getNextRid());
      setAttributes(newTag, {
        Id: curRid,
        Type: typeSl,
        Target: getTargetSlide(slideNum)
      });
      this.relationships.appendChild(newTag);
      newTag = this.presentationRels.createElement("p:sldId");
      setAttributes(newTag, {
        id: parseInt(this.currentRid + 1000, 10),
        "r:id": curRid
      });
      this.slideIdList.appendChild(newTag);
      this.xmlDocuments[getPathToRelSlide(slideNum)] = relationsSlides;
      this.existingSlides.push(slideNum);
      return map;
    }
  }, {
    ...
    value: function removeAllSlides() {
      this.addExtensionRels("image/png", "png");
      this.addExtensionRels(typeAR, "rels");
      this.addExtensionRels("application/xml", "xml");
      return this.existingSlides.forEach(this.removeSlide.bind(this));
    }
  }, {
    key: "loadSlides",
    value: function loadSlides() {
      this.contentTypes = this.xmlDocuments["[Content_Types].xml"];
      this.presentationRels = this.xmlDocuments["ppt/_rels/presentation.xml.rels"];
      this.presentation = this.xmlDocuments["ppt/presentation.xml"];
      this.types = this.contentTypes.getElementsByTagName("Types")[0];
      this.relationships = this.presentationRels.getElementsByTagName("Relationships")[0];
      this.slideIdList = this.presentation.getElementsByTagName("p:sldIdLst")[0];
      var relations = Array.prototype.slice.call(this.presentationRels.getElementsByTagName("Relationship"));
      this.ridArray = relations.map(function (tag) {
        return tag.getAttribute("Id");
      });
      this.existingSlides = compact(relations.map(function (tag) {
        return parseInt(tag.getAttribute("Target").replace(/.*slides\/slide(\d+)\.xml/, "$1"), 10);
      }));
      return this.existingSlides.sort();
    }
  }]);

  return SlidesRepository;
}();
...
function getPathToSlide(slideNum) {
  return "ppt/slides/slide".concat(slideNum, ".xml");
}

function getPathToRelSlide(slideNum) {
  return "ppt/slides/_rels/slide".concat(slideNum, ".xml.rels");
}

function getPathToNote(slideNum) {
  return "ppt/notesSlides/notesSlide".concat(slideNum, ".xml");
}

function getPathToNoteRel(slideNum) {
  return "ppt/notesSlides/_rels/notesSlide".concat(slideNum, ".xml.rels");
}

function getTargetSlide(slideNum) {
  return "slides/slide".concat(slideNum, ".xml");
}

var typeNS = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/notesSlide";
var typeSl = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide";
var typeAR = "application/vnd.openxmlformats-package.relationships+xml";
var typeANS = "application/vnd.openxmlformats-officedocument.presentationml.notesSlide+xml";
var typeAS = "application/vnd.openxmlformats-officedocument.presentationml.slide+xml";
module.exports = {
  getPathToSlide: getPathToSlide,
  getPathToRelSlide: getPathToRelSlide,
  getPathToNote: getPathToNote,
  getPathToNoteRel: getPathToNoteRel,
...
/******/ 	window.DocxtemplaterSlidesModule = __webpack_exports__;
/******/ 	
/******/ })()
;